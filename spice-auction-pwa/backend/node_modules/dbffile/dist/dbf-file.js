...
async function openDBF(path, opts) {
    let options = options_1.normaliseOpenOptions(opts);
    let fd = 0;
    try {
        ...
        if (options.readMode !== 'loose' && !file_version_1.isValidFileVersion(fileVersion)) {
        ...
        while (headerLength > 32 + fields.length * 32) {
            ...
            if (options.readMode !== 'loose') {
                field_descriptor_1.validateFieldDescriptor(field, fileVersion);
...
async function createDBF(path, fields, opts) {
    let options = options_1.normaliseCreateOptions(opts);
    let fd = 0;
    try {
...
function validateFieldDescriptors(fields, fileVersion) {
